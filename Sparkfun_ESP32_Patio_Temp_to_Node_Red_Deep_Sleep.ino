...
#define TIME_TO_SLEEP  55.5        /* Time ESP32 will go to sleep (in seconds) */
...
//**********Declaring Variables**********
int d, mo, y;
int WiFiLed = 5;//**********Pin 2 is the on board blue LED**********
...
RTC_DATA_ATTR int maxL, s, ssec, m, h;
RTC_DATA_ATTR double avgT, maxT, maxTToday, avgTCalc, avgH, maxH, maxHToday, avgHCalc, avgL, avgLCalc, avgP, maxP, avgPCalc, avgRssi, maxRssi, avgRssiCalc;
RTC_DATA_ATTR double minT = 120;
RTC_DATA_ATTR double minH = 100;
RTC_DATA_ATTR double minP = 1100;
RTC_DATA_ATTR double minRssi = -100;
RTC_DATA_ATTR double minTToday = 120;
RTC_DATA_ATTR double minHToday = 100;
...
void calcAvgRssi(int rssi)
{
  if (!isnan(rssi) || rssiString == "nan")
  {
    ...
    {
      avgRssi = avgRssiCalc / bootCounter;
    }
...
void printElapsedTime()
{
  ...
  {
    s = (bootCounter - 1) * 60 + (timeinfo.tm_sec - ssec);
  }
  second = s;
  if(s > 59)
  {
...
double printBattVoltage()
{
  ...
  vBatt = vBatt / 847;//Calibration factor
...
//  client.publish(timeStampTopic, timeStamp);
